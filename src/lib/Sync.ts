...
Dexie.Syncable.registerSyncProtocol("websocket", {
  ...
  sync: function (context, url, _options, baseRevision, syncedRevision, changes, partial, applyRemoteChanges, onChangesAccepted, onSuccess, onError) {
    ...
    // The following vars are needed because we must know which callback to ack when server sends its ack to us.
    let requestId = 0;
    let acceptCallbacks = {};
    ...
    // sendChanges() method:
    function sendChanges(changes: IDatabaseChange[], baseRevision, partial: boolean, onChangesAccepted) {
      ++requestId;
      ...
      //  However, this example shows how to deal with the WebSocket to fulfill the API.
      ...
      try {
        ...
        if (requestFromServer.type == "changes") {
          ...
          if (isFirstRound && !requestFromServer.partial) {
            // Since this is the first sync round and server said we've got all changes - now is the time to call onsuccess()
            onSuccess({
